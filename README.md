...
For clarity, the arguments can also be named when the function is called, then the order of the arguments doesn't matter.
...
Sometimes being able to pass an arbitrary number of arguments can be useful, especially when another function is called within a wrapper function. Here's an example:
...
Here's a less trivial example - this function takes a string array as the argument, ensures all letters in the string array are lower-case, and makes use of "regular expressions" or "regex"" to find and replace all spaces and punctuation marks with an underscore:
