...
In short, using functions can make your code easier to read, easier to write, easier to QA and easier to modify. So really the question is why would you not?
...
We're using the function `function()` to create a function and assign it to the name `function_name`.
...
3. And the function *environment* (where the function was created)

*Arguments* are variables used inside the function body which we can set each time we call the function. The function *body* is where we put the code. The function's *environment* determines what variables and other functions it has access to. This is closely linked to the concept of scoping, something you can read more about [here](https://bookdown.org/rdpeng/rprogdatascience/scoping-rules-of-r.html).
...
To find out more about writing packages, check out the further reading below.
