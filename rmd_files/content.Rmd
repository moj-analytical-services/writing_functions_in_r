...
This function `pythagoras()` calculates the length of a hypotenuse triangle, given the other two lengths. However, it will return a result for negative numbers, but this is not ideal behavior in the context.
...
We can add assertion statements to prevent this undesirable behaviour. We can also combine the two assertions into one, to avoid repetition.

```{r, purl=purl_example_code}
# Function to calculate the length of a hypotenuse 
pythagoras_3 <- function(a, b) {
