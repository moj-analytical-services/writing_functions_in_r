...
```{r, include=show_solution, purl=purl_solution}
...
```{r, include=show_solution, purl=purl_solution, eval=FALSE}
...
```{r message=F, warning=F, purl=show_code}
# Load packages
library(s3tools)
library(dplyr)
library(stringr)
...
```{r, include=FALSE, purl=show_code}
# This is code is included in case Exercise 3 wasn't completed above
colnames(prosecutions) <- generalise_names(colnames(prosecutions))
```
...
```{r fig.width=10, include=show_solution, purl=purl_solution}
# Exercise 6 solution
