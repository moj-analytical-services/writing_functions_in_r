...
Sometimes it can be useful to include helpful error messages in functions, by anticipating the sorts of variables that could be input. What happens if you try to use a non-integer or a string as an argument in the above example? Try the running the function with the following arguments:
...
`stop()` halts execution of the function and prints an error message to the console; alternatively you can use `warning()`, which prints a warning message to the console but does not stop the execution of the function.
...
```{r}
# Note: if you are running this code from 'example_code.R', then you
# should replace the following with `source("rmd_files/functions.R")`
source("functions.R")
```
