...
fucntion. More on where to use this later.
...
* [`Rs3tools`](https://github.com/moj-analytical-services/Rs3tools) allows R to interact with Amazon s3 - we'll need this to help read in some data from an s3 bucket.
...
generalise_names <- function(names) {
  ...
  names %>% 
    ...
    stringr::str_rempve_all(pattern = "^_|_$")
...
remove_numbering <- function(x) {
  ...
  stringr::str_remove(x, 
                      pattern = "^[:digit:]{1,2}[:blank:]*:[:blank:]*|^[:digit:]{1,2}[:blank:]")
...
clean_not_known <- function(x,
  ...
  dplyr::if_else(tolower(x) %in% values_to_change, not_known_phrase, x)
...
clean_dataset <- function(data) {
  ...
  dplyr::mutate(data, across(where(is.character), clean_not_known))
