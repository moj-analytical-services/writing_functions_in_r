...
**Note:** It’s best practice for your function to either return an object or have a side-effect, but not both (with the exception of simple print statements).
...
* `botor` allows R to interact with Amazon s3 - we'll need this to help read in some data from an s3 bucket.
...
In both development and deployment, the effects of functions can differ from that which is expected.

Problem 1: If you have experienced such problems while writing custom functions in the above exercises, you may have seen an error message. Any error messages that you have seen while executing a custom function will have originated from the functions that your custom function calls. They are by nature generic. Sometimes, these error messages will be easy for you to understand, and therefore see what's gone wrong, but because they are generic, they can sometimes be confusing.

Problem 2: You may also have noticed that a function you wrote wasn't doing what you wanted because the output was surprising. This kind of thing is more pernicious, because you may not notice the problem straight away. When you're trying to debug a complex network of functions, the root cause may be obscure if it doesn't generate an error at all.

Solution: You can make life easier by adding _assertions_ (calls to assertion functions).

An assertion function call accepts a condition, and stops the function execution if that condition is not met. You can use them to check that inputs, outputs and intermediate objects are as expected. You can also provide them with custom error messages that are specific to your custom function, and helpful to the user.

A good package - developed by the RStudio (Posit) people - is `assertthat`. It is extremely easy to use, in particular the function `assertthat::assert_that()`, which we will focus on.

The first - and only essential - argument that `assertthat::assert_that()` takes is an expression that returns either `TRUE` or `FALSE`. It's very useful for testing input, output and intermediate objects.

This function `pythagoras()` calculates the length of a right-angled triangle's hypotenuse side, given the lengths of the other two sides. However, it will return a result for negative numbers, and this is not ideal behavior in the context.
...
That's great, but we could make things more informative. The second most useful argument that `assertthat::assert_that()` takes is `msg`. This is a string that will be output as part of the error message, overriding the default, e.g. 
...
1. Add the below assertion to `pythagorus()` in the correct place to cause it to fail when called with `pythagoras(2, 3)`.
3. Add a message that would tell the developer why their function is not passing the assertion.
2. Modify the function so that the assertion passes and the function succeeds.
...
```{r, include=show_solution, purl=purl_solutions}
# Exercise 7 solution
# Function to return hypotenuse length rounded to whole number
pythagoras <- function(a, b) {
  ...
  # assertion statement checks product before returning to user
  assertthat::assert_that(
    c %% 1 == 0,
    msg = "value to be returned is not a whole number")
