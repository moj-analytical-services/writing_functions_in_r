...
If you would like to follow along with the code run during this session (recommended if you indend to do the exercises) then open the script "example_code.R" in RStudio.
...
Sometimes it can be useful to include helpful error messages in functions, e.g. by anticipating the sorts of variables that could input. What happens if you try to use a non-integer or a string as an argument in the above example? Try the running the function with the following arguments:
