...
Then we can use the `mutate_if()` function from `dplyr` to apply the `remove_numbering()` function to columns in the `prosecutions` dataframe. The `mutate_if()` function will apply the specified function to all columns where a particular condition is met, and in this case the condition `is.character` requires that the column contains strings.
...
sum_group <- function(df, group_cols, sum_col) {
  ...
  summary <- df %>%
    dplyr::group_by(.data[[group_cols]]) %>%
...
The way that the Tidyverse packages handle NSE has evolved over the past few years, so new methods of addressing this issue may be introduced in the future. See the appendix for some examples of alternative methods to tackle this problem.
