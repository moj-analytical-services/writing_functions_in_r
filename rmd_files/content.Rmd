...
If you would like to follow along with the code run during this session (recommended if you intend to do the exercises) then open the script "example_code.R" in RStudio.
...
sure object you want to pipe (usually a dataframe) is the first argument for your function.
...
Functions can return different outputs depending on some condition, using the `return()` command. In this function the condition is `x < 0`, and the condition evaluates to either `TRUE` or `FALSE`:
...
* To find out more about if statements & conditional execution, see [here](https://r4ds.had.co.nz/functions.html#conditional-execution)
...
**Hint:** 
if else statements aren't vectorised so you'll need to switch to using 
...
In this dataset, some of the columns contain values with a number along with a category; for example, the `age_group` column contains categories like "01: Juveniles" rather than just "Juveniles". These numbers might be undesirable, so we can write a function to remove these:
...
Then we can use the `modify_if()` function from `purrr` to apply the `remove_numbering()` function to all columns in the `prosecutions` dataframe, with the condition that the column must contain strings.
...
In this function we've included some default values, so by default missing/NA values are replaced, and any strings that match "n/a", "not known", "unknown", or "not stated" are replaced. The default replacement value is "Not known".
...
Notice how the `dplyr` functions `group_by()` and `summarise()` require that the column names are not enclosed in quotation marks, meaning that they are not passed as strings: this behaviour is known as non-standard evalution, and will be important later.
...
Modify the function `prosecutions_graph` to use `theme_classic()` as the theme, rather than `theme_grey()`. Produce a plot of with a breakdown by `"offence_group"`.
...
prosecutions_graph(prosecutions, breakdown = "offence_group")
...
Often processing data requires manipulating dates and times. For example, if we wanted to extract the prosecutions from the year up to a particular date, we could use a function like:
...
Then just use `source("my_functions_script.R")` (with `my_functions_script.R` replaced with the name of your script) to run the code and make your functions available to you in the current session. As with loading packages, it's best to do this at the top of your script.
